...
namespace SocialMediaDashboard.W8.Common {
    /// <summary>
    ///     Usage:
    ...
    public class Properties : DependencyObject {
        public static readonly DependencyProperty HtmlProperty =
            DependencyProperty.RegisterAttached("Html", typeof(string),
                typeof(Properties), new PropertyMetadata(null, HtmlChanged));
        ...
        /// <param name="d"></param>
        /// <param name="e"></param>
        private static void HtmlChanged(DependencyObject d,
            ...
            var baselink = App.ServerEndpoint;

            var blocks = GenerateBlocksForHtml(xhtml, baselink);
                ...
                foreach (var img in doc.DocumentNode.Descendants("img"))
                    if (!img.Attributes["src"].Value.StartsWith("http"))
                        img.Attributes["src"].Value =
                            baselink + img.Attributes["src"].Value;
        ...
        private static Inline GenerateImage(HtmlNode node) {
            ...
            try {
                var iui = new InlineUIContainer();
                var sourceUri =
                    WebUtility.HtmlDecode(node.Attributes["src"].Value);
                var img = new Image {
                    Source =
                        new BitmapImage(new Uri(sourceUri, UriKind.Absolute))
                };
        ...
        private static void img_ImageFailed(object sender,
            ExceptionRoutedEventArgs e) {
            var i = 5;
        }

        private static void img_ImageOpened(object sender, RoutedEventArgs e) {
        ...
        private static Inline GenerateHyperLink(HtmlNode node) {
            ...
            if (node.ParentNode != null &&
                (node.ParentNode.Name == "li" || node.ParentNode.Name == "LI"))
                hb.Style = (Style) Application.Current.Resources["RTLinkLI"];
            else if ((node.NextSibling == null ||
                      string.IsNullOrWhiteSpace(node.NextSibling.InnerText)) &&
        ...
        private static Inline GenerateIFrame(HtmlNode node) {
            try {
                ...
                var iui = new InlineUIContainer();
                var ww = new WebView {
                    Source = new Uri(node.Attributes["src"].Value,
                        UriKind.Absolute),
        ...
        private static async Task<string> ConvertHtmlToXamlRichTextBlock(
            string xhtml) {
            ...
            if (Html2XamlProcessor == null) {
                ...
                using (var stream = assembly.GetManifestResourceStream(
                    "SocialMediaDashboard.W8.Common.RichTextBlockHtml2Xaml.xslt")
                ) {
            ...
            // Apply XSLT to XML
            var xaml =
                Html2XamlProcessor.TransformToString(xhtmlDoc.FirstChild);
            return xaml;
        }
        ...
        /// <returns></returns>
        private static async Task<string> ConvertHtmlToXamlRichTextBlock2(
            string xhtml) {
            var xaml =
                "<?xml version=\"1.0\"?><RichTextBlock xmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\" xmlns:x=\"http://schemas.microsoft.com/winfx/2006/xaml\">";
            ...
            switch (node.Name) {
                ...
                case "H1":
                    result = string.Format(
                        "<Paragraph FontSize=\"30\">{0}</Paragraph>",
                ...
                case "H3":
                    result = string.Format(
                        "<Paragraph><Bold>{0}</Bold></Paragraph>",
                        CleanText(node.InnerText));
                    break;
                case "ul":
                case "UL":
                    result = string.Format(
                        "<Paragraph Margin=\"20,0,0,0\"><LineBreak />{0}</Paragraph>",
                        GenerateBlockForChildren(node));
                    break;
                default:
                    ...
                    //    result = "";
                    //}
                    result = string.Format("<Paragraph>{0}</Paragraph>",
                        GenerateBlockForChildren(node));
                    break;
            }
        ...
        private static string GenerateBlockForNode_old(HtmlNode node) {
            ...
            switch (node.Name) {
                case "div":
                    result = string.Format("<Span>{0}</Span>",
                        GenerateBlockForChildren(node));
                    break;
                case "p":
                case "P":
                    result = string.Format(
                        "<Span><LineBreak />{0}<LineBreak /></Span>",
                        GenerateBlockForChildren(node));
                    break;
                case "img":
                case "IMG": {
                    ...
                    //else
                    result =
                        "<Span><InlineUIContainer><Image Style=\"{StaticResource RTImage}\" Source=\"" +
                        node.Attributes["src"].Value +
                        "\"></Image></InlineUIContainer></Span>";
                }
                    break;
                case "a":
                case "A":
                    if (node.ChildNodes.Count == 1 &&
                        ...
                        if (node.ParentNode != null &&
                            (node.ParentNode.Name == "li" ||
                             node.ParentNode.Name == "LI"))
                            result =
                                "<Span><InlineUIContainer><HyperlinkButton Style=\"{StaticResource RTLinkLI}\" NavigateUri=\"" +
                                node.Attributes["href"].Value + "\">" +
                                CleanText(node.InnerText) +
                                "</HyperlinkButton></InlineUIContainer></Span>";
                        else if ((node.NextSibling == null ||
                                  string.IsNullOrWhiteSpace(node.NextSibling
                                      .InnerText)) &&
                                 (node.PreviousSibling == null ||
                                  string.IsNullOrWhiteSpace(node.PreviousSibling
                                      .InnerText)))
                            result =
                                "<Span><InlineUIContainer><HyperlinkButton Style=\"{StaticResource RTLinkOnly}\" NavigateUri=\"" +
                                node.Attributes["href"].Value + "\">" +
                                CleanText(node.InnerText) +
                                "</HyperlinkButton></InlineUIContainer></Span>";
                        else
                            result =
                                "<Span><InlineUIContainer><HyperlinkButton Style=\"{StaticResource RTLink}\" NavigateUri=\"" +
                                node.Attributes["href"].Value + "\">" +
                                CleanText(node.InnerText) +
                                "</HyperlinkButton></InlineUIContainer></Span>";
                    }
                ...
                case "LI":
                    result =
                        "<Span><InlineUIContainer><Ellipse Style=\"{StaticResource RTBullet}\"/></InlineUIContainer>" +
                        GenerateBlockForChildren(node) + "<LineBreak /></Span>";
                    break;
                case "b":
                case "B":
                    result = string.Format("<Bold>{0}</Bold>",
                        GenerateBlockForChildren(node));
                    break;
                case "i":
                case "I":
                    result = string.Format("<Italic>{0}</Italic>",
                        GenerateBlockForChildren(node));
                    break;
                case "u":
                case "U":
                    result = string.Format("<Underline>{0}</Underline>",
                        GenerateBlockForChildren(node));
                    break;
                case "br":
                case "BR":
